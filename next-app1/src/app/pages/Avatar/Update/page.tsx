...
export default function Update() {
    ...
    const handleSubmit = async () => {
        ...
        if (file) {
            ...
            if (id && image64){
                const avatar: IAvatar = (await AvatarService.GetById(id)).data!;
                let img: string = image64;
                avatar.image = img.replace("data:image/png;base64,", "");
                const response = await AvatarService.Update(avatar);
                if(!response.errors){
